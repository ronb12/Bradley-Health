...
let distanceLog = JSON.parse(localStorage.getItem('rollDistances')) || [];
...
function logDistance(dist, show = true) {
  const date = new Date().toISOString().split('T')[0];
  distanceLog.unshift({ date, distance: dist.toFixed(2) });
  localStorage.setItem('rollDistances', JSON.stringify(distanceLog));
  if (show) renderLog();
}
