...
class WeightLossManager {
  ...
  displayMealPlan(mealPlan) {
    ...
    if (!mealPlanCard.querySelector('.print-plan-btn')) {
      ...
      shareButton.className = 'btn btn-success share-plan-btn';
      shareButton.innerHTML = '📤 Share Plan';
      shareButton.onclick = () => this.shareMealPlan(mealPlan);
      mealPlanCard.appendChild(shareButton);
    }
  }
  ...
  shareMealPlan(mealPlan) {
    if (!mealPlan || !mealPlan.days) {
      this.showToast('No meal plan available to share', 'error');
      return;
    }

    // Create shareable content
    const shareContent = this.createShareableContent(mealPlan);
    
    // Check if Web Share API is available (mobile devices)
    if (navigator.share) {
      navigator.share({
        title: 'My Bradley Health Meal Plan',
        ...
        this.showShareModal(shareContent);
      });
    } else {
  ...
  createShareableContent(mealPlan) {
    // Collect all beverages (smoothies and juices)
    const beverages = [];
    mealPlan.days.forEach((day, dayIndex) => {
      Object.entries(day.meals).forEach(([mealType, meal]) => {
        if (meal.ingredients && meal.instructions && meal.ingredients.length > 0) {
          ...
          if (isJuice || isSmoothie) {
            beverages.push({
              day: dayIndex + 1,
              mealType: mealType,
              beverageType: isJuice ? 'juice' : 'smoothie',
              name: meal.name,
              ingredients: meal.ingredients,
              instructions: meal.instructions,
              benefits: meal.benefits || ''
    ...
    // Create summary text
    const summary = `🍽️ My Bradley Health Meal Plan\n\n` +
      `📊 Daily Calories: ${mealPlan.dailyCalories}\n` +
      `📅 Plan Duration: ${mealPlan.totalDays} days\n` +
      `🥤🍹 Beverages: ${beverages.length} (${beverages.filter(b => b.beverageType === 'smoothie').length} smoothies, ${beverages.filter(b => b.beverageType === 'juice').length} juices)\n\n` +
      `Generated by Bradley Health App`;

    // Create detailed content for copying
    const detailedContent = this.createDetailedShareContent(mealPlan, beverages);
  ...
  createDetailedShareContent(mealPlan, beverages) {
    let content = `🍽️ MY BRADLEY HEALTH MEAL PLAN\n`;
    content += `Generated by Bradley Health App\n`;
    content += `Date: ${new Date().toLocaleDateString()}\n`;
    content += `\n📊 PLAN SUMMARY:\n`;
    content += `• Daily Calories: ${mealPlan.dailyCalories}\n`;
    content += `• Protein: ${mealPlan.macros.protein}g\n`;
    content += `• Carbs: ${mealPlan.macros.carbs}g\n`;
    content += `• Fat: ${mealPlan.macros.fat}g\n`;
    content += `• Plan Duration: ${mealPlan.totalDays} days\n`;
    content += `\n🥤🍹 BEVERAGES (${beverages.length} total):\n`;

    beverages.forEach((beverage, index) => {
      content += `\n${index + 1}. ${beverage.name.toUpperCase()} (${beverage.beverageType.toUpperCase()})\n`;
      content += `   Day ${beverage.day} - ${beverage.mealType}\n`;
      content += `   Ingredients:\n`;
      beverage.ingredients.forEach(ingredient => {
        content += `   • ${ingredient}\n`;
      });
      content += `   Instructions: ${beverage.instructions}\n`;
      if (beverage.benefits) {
        content += `   Benefits: ${beverage.benefits}\n`;
      }
    });

    content += `\n📋 SAMPLE MEALS (First 7 days):\n`;
    mealPlan.days.slice(0, 7).forEach((day, dayIndex) => {
      content += `\nDay ${dayIndex + 1}:\n`;
      Object.entries(day.meals).forEach(([mealType, meal]) => {
        content += `  ${mealType.charAt(0).toUpperCase() + mealType.slice(1)}: ${meal.name} (${meal.calories} cal)\n`;
      });
    });

    content += `\n💡 TIPS:\n`;
    content += `• Drink beverages immediately for best taste and nutrition\n`;
    content += `• Store ingredients properly for freshness\n`;
    content += `• Adjust portions based on your needs\n`;
    content += `• Consult with healthcare provider for personalized advice\n`;
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="share-summary">
            <h4>📊 Plan Summary</h4>
            <p>${shareContent.summary.replace(/\n/g, '<br>')}</p>
          </div>
          ...
          <div class="share-options">
            ...
            <div class="share-buttons">
              <button class="btn btn-primary" onclick="navigator.clipboard.writeText('${shareContent.summary.replace(/'/g, "\\'")}').then(() => { this.showToast('Summary copied to clipboard!', 'success'); })">
                📋 Copy Summary
              </button>
              
              <button class="btn btn-secondary" onclick="navigator.clipboard.writeText('${shareContent.detailed.replace(/'/g, "\\'")}').then(() => { this.showToast('Detailed plan copied to clipboard!', 'success'); })">
                📄 Copy Full Plan
              </button>
              
              <button class="btn btn-info" onclick="this.shareBeveragesOnly('${JSON.stringify(shareContent.beverages).replace(/'/g, "\\'")}')">
                🥤🍹 Share Beverages Only
              </button>
            </div>
            ...
            <div class="share-methods">
              ...
              <div class="method-buttons">
                <button class="btn btn-outline-primary" onclick="window.open('mailto:?subject=My Bradley Health Meal Plan&body=${encodeURIComponent(shareContent.summary)}')">
                  📧 Email
                </button>
                <button class="btn btn-outline-success" onclick="window.open('sms:?body=${encodeURIComponent(shareContent.summary)}')">
                  💬 SMS
                </button>
                <button class="btn btn-outline-info" onclick="window.open('https://wa.me/?text=${encodeURIComponent(shareContent.summary)}')">
                  📱 WhatsApp
                </button>
              </div>
            </div>
  ...
  shareBeveragesOnly(beveragesJson) {
    try {
      const beverages = JSON.parse(beveragesJson);
      let content = `🥤🍹 MY BRADLEY HEALTH BEVERAGES\n`;
      content += `Generated by Bradley Health App\n`;
      content += `Date: ${new Date().toLocaleDateString()}\n\n`;

      beverages.forEach((beverage, index) => {
        content += `${index + 1}. ${beverage.name.toUpperCase()} (${beverage.beverageType.toUpperCase()})\n`;
        content += `   Day ${beverage.day} - ${beverage.mealType}\n`;
        content += `   Ingredients:\n`;
        beverage.ingredients.forEach(ingredient => {
          content += `   • ${ingredient}\n`;
      ...
      content += `💡 TIPS:\n`;
      content += `• Drink immediately for best taste and nutrition\n`;
      content += `• Use fresh, organic ingredients when possible\n`;
      content += `• Clean juicer/blender immediately after use\n`;
      content += `• Store ingredients properly for freshness\n`;

      navigator.clipboard.writeText(content).then(() => {
        this.showToast('Beverages copied to clipboard!', 'success');
      });
    } catch (error) {
      console.error('Error sharing beverages:', error);
      this.showToast('Error sharing beverages', 'error');
    }
  }
}
